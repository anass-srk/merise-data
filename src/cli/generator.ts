...
export function generateMLD(model: Model, filePath: string, destination: string | undefined): string {
  ...
  //     if(l.filter(v => v.mult < 2).length == 2){
  //       let slave = l[0]
  //       let master = l[1]
  //       if(l[0].mult == 0){
  //         slave = l[0]
  ...
  //         master = l[0];        
  //       }
  //       const myatts = master.entity.attributes.filter(att => att.is_primary).map<MyAttribute>(att => ({
  //         is_primary: true,
  //         is_foreign: true,
  //         name: att.name,
  //         type: att.type
  //       }))
  //       slave.entity.attributes.push(...myatts)
  //     }else {
  ...
  //   Array.from(new_ents.values()).map<string>(e => `object ${e.name} {\n} `).join('\n') + '\n' + 
  //   new_rels.map(r => `${r.ea} ${r.am}--${r.bm} ${r.eb}`).join('\n')
  // );
  return generatedFilePath;
}